namespace ScopicAuctionSystem.Infrastructure.Identity
{
    ...
    public class UserManagerService : IUserManager
    {
        ...
        public async Task<(Result Result, string UserId)> SignIn(string email, string password)
        {
            ...
            if (user == null) return (Result.Failure(ExceptionMessages.User.InvalidCredentials), null);
        ...
        public async Task CreateRoleAsync(IdentityRole role)
        {
            ...
            if (!roleExist) await roleManager.CreateAsync(new IdentityRole(AppConstants.AdministratorRole));
        }
        ...
        {
            var user = await GetDomainUserByEmailAsync(email);
            if (user == null) return Result.Failure(ExceptionMessages.User.UserNotFound);
            ...
            if (removedUserRefreshToken != null) removedUserRefreshToken.Invalidated = true;
